...
from pulp_glance.common import constants, error_codes
...
def get_root_publish_directory(config):
    """
    The publish directory for the glance image plugin

    :param config: Pulp configuration for the distributor
    :type  config: pulp.plugins.config.PluginCallConfiguration
    :return: The publish directory for the glance image plugin
    :rtype: str
    """
...
def get_redirect_url(config, repo):
    ...
    if redirect_url:
        ...
        redirect_url = 'https://%s/pulp/glance/%s/' % (server_name, repo.id)
