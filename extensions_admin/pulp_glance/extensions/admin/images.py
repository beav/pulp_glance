...
def get_formatter_for_type(type_id):
    ...
    if type_id != constants.IMAGE_TYPE_ID:
        raise ValueError(_("The glance image formatter can not process %s units.") % type_id)
...
class ImageCopyCommand(UnitCopyCommand):
    ...
    def get_formatter_for_type(type_id):
        """
        Returns a method that can be used to format the unit key of a glance image
...
class ImageRemoveCommand(UnitRemoveCommand):
    """
    Class for executing unit remove commands for glance image units
    """
...
class ImageSearchCommand(DisplayUnitAssociationsCommand):
    ...
    def run(self, **kwargs):
        ...
        :param kwargs: the search parameters for finding glance images
        :type kwargs: dict
        """
